...
void main() {
  test(
    'engine_backend true rejects a cross-OS build instead of using stub',
    () {
  ...
  test(
    'engine_backend true rejects a cross-architecture build instead of stub',
    () {
  ...
  test('engine_backend rejects non-boolean configuration before any build', () {
    return expectLater(
      _runHook(
        targetOS: OS.current,
      ...
      throwsA(
        isA<FormatException>().having(
          (error) => error.message,
          'message',
          contains('engine_backend must be a boolean'),
        ),
      ),
    );
  });
  ...
  return testCodeBuildHook(
    ...
    userDefines: PackageUserDefines(
      workspacePubspec: PackageUserDefinesSource(
        defines: {'engine_backend': engineBackend},
        basePath: Directory.current.uri,
      ),
    ),
    check: (_, _) => fail('A refused engine build must not produce an asset.'),
  );
}
