...
import 'package:aonw_engine_client/aonw_engine_client_bindings.dart'
    as bindings;
import 'package:aonw_engine_client/src/api.dart';
import 'package:aonw_engine_client/src/native_identity.dart';
import 'package:ffi/ffi.dart';

AonwNativeIdentity get aonwEngineClientIdentity {
  try {
...
bool get aonwEngineClientAvailable => aonwEngineClientIdentity.isCompatible;

Future<AonwEngineSession?> createAonwEngineSession() async {
  if (!aonwEngineClientIdentity.isCompatible) return null;
  return _NativeAonwEngineSession.start();
}

final class _NativeAonwEngineSession implements AonwEngineSession {
  _NativeAonwEngineSession._(this._commands, this._responses) {
  ...
  static Future<_NativeAonwEngineSession> start() async {
    ...
    if (commands == null) {
      responses.close();
      throw StateError(result.error ?? 'Engine session startup failed.');
    }
    return _NativeAonwEngineSession._(commands, responses);
  }

  @override
  Future<String> requestJson(String request) {
    if (_closed) throw StateError('Engine session is closed.');
...
void _sessionWorker(_WorkerPorts ports) {
  ...
  if (session == ffi.nullptr) {
    ports.ready.send(
      const _WorkerReady.failure('Engine session allocation failed.'),
    );
    return;
  }
...
String _dispatch(ffi.Pointer<ffi.Void> session, String request) {
  ...
  try {
    ...
    if (response == ffi.nullptr) {
      throw StateError('Engine returned no response.');
    }
    try {
      ...
      if (data == ffi.nullptr && length != 0) {
        throw StateError('Engine returned an invalid response.');
      }
