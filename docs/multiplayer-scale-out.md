...
canonical engine state and every durable game record; each Serverpod instance
loads the locked match row, invokes the same engine runtime, and commits the
...
- Deploy the exact same Dart, engine, protocol, map, and ruleset artifacts to
...
The deployment-image and readiness procedure is recorded in
