...
as 160 model metres. This is a visual design convention, not a claim about the
geographic area represented by a gameplay tile. Native heights, map radius,
water, picking and saved terrain workspaces are unchanged.

`map_units_per_model_metre = 2 * hex_radius_meters / city_hex_diameter`

At the current 10-unit radius, the conversion is 0.125. A 12 m tree is 1.5 map
units before variation, instead of the old 5-unit tree. A future 10 m house is
1.25 map units; a 2 m rock is 0.25. The conversion is uniform and shared with
...
the existing 120,000-candidate/20,000-tree budgets can make it coarser on large
...
- outer band: natural vegetation follows the existing reference and terrain.
